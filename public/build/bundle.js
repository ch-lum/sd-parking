...
var app = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			t1 = space();
    			input = element("input");
    			add_location(label, file, 193, 2, 6318);
    			attr_dev(input, "id", "slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "1");
    			attr_dev(input, "max", "1440");
    			add_location(input, file, 194, 2, 6351);
    			attr_dev(div, "class", "overlay");
    			add_location(div, file, 192, 4, 6293);
    			add_location(main, file, 191, 0, 6281);
    		},
    ...
    function scaleDifferenceColor(difference, maxDifference = 50) {
    	const scaleColor = d3.scaleLinear().domain([-1 * maxDifference, maxDifference]).range(["blue", "orange"]);

    	// console.log((scaleColor(difference)))
    	return scaleColor(difference);
    }
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	map.on("load", () => {
    		...
    		map.addLayer({
    			...
    			type: "line",
    			source: "boston_route",
    			paint: { "line-color": "#ff9999", "line-width": 3 }
    		});
    	...
    	function update_station_markers() {
    		station_markers.transition().duration(1000).attr("r", function (d) {
    			...
    			let trafficDifference = arrivals[d["station_id"]] - departures[d["station_id"]];
    			return scaleDifferenceColor(trafficDifference);
